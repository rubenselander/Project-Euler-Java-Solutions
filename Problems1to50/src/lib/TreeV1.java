...
public class TreeV1 {
	...
	/**
	 * This class implements the nodes that will go on the Binary Tree. They consist
	 * of the data in them, the node to the left, the node to the right, and the
	 * parent from which they came from.
	 *
	 * @author Unknown
	 */
	static class Node {
	 ...
	 * @return The node if it finds it, otherwise returns the parent
	 */
	public Node findEmptyParent() {
		int maxLevel = nodes.get(nodes.size() - 1).level;
		if (maxLevel == 0) {
			return root;
		}

		for (Node n : nodes) {
			if (n.level == maxLevel - 1 && n.left == null || n.right == null) {
				return n;
			}
		}
	...
	public void put(int value) {
		...
		if (root == null) {
			...
			if (parent.left == null) {
				...
				System.out.println(newNode.data + " added as left child of " + parent.data);
				return;
			} else {
				...
				System.out.println(newNode.data + " added as right child of " + parent.data);
				return;
			}
		}
	}
	...
	public boolean remove(int value) {
		...
		if (temp.right == null && temp.left == null) {
			...
			// If the successor has a right child, the child's grandparent is it's new
			// parent
